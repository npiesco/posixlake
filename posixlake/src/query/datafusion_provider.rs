...
#[derive(Debug)]
pub struct PosixLakeTableProvider {
    inner: Arc<MemTable>,
}

impl PosixLakeTableProvider {
...
impl TableProvider for PosixLakeTableProvider {
...
mod tests {
    ...
    fn test_table_provider_creation() {
        ...
        let provider = PosixLakeTableProvider::new(schema.clone(), vec![batch]).unwrap();
    ...
    async fn test_table_provider_scan_all_columns() {
        ...
        let provider = PosixLakeTableProvider::new(schema.clone(), vec![batch.clone()]).unwrap();
    ...
    async fn test_table_provider_select_specific_columns() {
        ...
        let provider = PosixLakeTableProvider::new(schema, vec![batch]).unwrap();
    ...
    async fn test_table_provider_multiple_batches() {
        ...
        let provider = PosixLakeTableProvider::new(schema, vec![batch1, batch2]).unwrap();
    ...
    async fn test_table_provider_join() {
        ...
        let provider1 = PosixLakeTableProvider::new(schema1, vec![batch1]).unwrap();
        let provider2 = PosixLakeTableProvider::new(schema2, vec![batch2]).unwrap();
