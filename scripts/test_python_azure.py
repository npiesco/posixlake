#!/usr/bin/env python3
"""
Integration tests for posixlake Python Azure bindings via UniFFI.

Runs against a local Azurite instance on 127.0.0.1:10000 and exercises the Python
...
from posixlake import DatabaseOps, Field, HealthStatus, PosixLakeError, AzureConfig, Schema

AZURE_ENDPOINT = "http://127.0.0.1:10000"
AZURE_ACCOUNT_NAME = "devstoreaccount1"
AZURE_ACCOUNT_KEY = (
    "Eby8vdM02xNOcqFlqUwJPLlmEtlCDXJ1OUzFT50uSRZ6IFsuFq2UVErCz4I6tq"
    "/K1SZFPTOtr/KBHBeksoGMGw=="
)
AZURE_CONTAINER = "posixlake-test"
...
def azurite_reachable():
    try:
        sock = socket.create_connection(("127.0.0.1", 10000), timeout=2)
        sock.close()
        return True
    except OSError:
        return False
...
def unique_azure_path():
    import uuid
    return f"az://plt{uuid.uuid4().hex}"
...
def test_create_with_azure(results):
    section("Test 1: Create Azure Database")
    try:
        db = DatabaseOps.create_with_azure(unique_azure_path(), iot_schema(), azure_config())
        if db is None:
            raise RuntimeError("DatabaseOps.create_with_azure returned None")
        schema = db.get_schema()
        field_names = [field.name for field in schema.fields]
        assert field_names == ["id", "sensor", "reading", "location"]
        results.ok("create_with_azure")
        results.ok("schema round-trip")
    except Exception as e:
        results.fail("create_with_azure", str(e))


def test_create_with_bad_endpoint(results):
    section("Test 2: Bad Azure Endpoint")
    try:
        ...
        try:
            DatabaseOps.create_with_azure(unique_azure_path(), iot_schema(), bad_config)
...
def test_open_with_azure(results):
    section("Test 3: Open Azure Database")
    try:
        path = unique_azure_path()
        db = DatabaseOps.create_with_azure(path, iot_schema(), azure_config())
        db.insert_json(json.dumps([
            {"id": 1, "sensor": "temp_01", "reading": 23.5, "location": "rack_A"}
        ]))
        reopened = DatabaseOps.open_with_azure(path, azure_config())
        rows = json.loads(reopened.query_json("SELECT * FROM data"))
        assert len(rows) == 1
        assert rows[0]["sensor"] == "temp_01"
        results.ok("open_with_azure")
    except Exception as e:
        results.fail("open_with_azure", str(e))


def test_health_check(results):
    section("Test 4: Azure Health Check")
    try:
        db = DatabaseOps.create_with_azure(unique_azure_path(), iot_schema(), azure_config())
        db.insert_json(json.dumps([
            {"id": 1, "sensor": "temp_01", "reading": 23.5, "location": "rack_A"}
...
def main():
    print("=" * 70)
    print("  posixlake Python Azure integration test")
    print("=" * 70)

    results = TestResult()

    if not azurite_reachable():
        results.skip("Azurite dependency", "Azurite not reachable on 127.0.0.1:10000")
        success = results.summary()
        sys.exit(0 if success else 1)

    test_create_with_azure(results)
    test_create_with_bad_endpoint(results)
    test_open_with_azure(results)
