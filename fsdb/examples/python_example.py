#!/usr/bin/env python3
"""
FSDB Python Bindings - Comprehensive Example

This example demonstrates all FSDB features available from Python via UniFFI bindings.
...
Or from source:
1. Build: cd fsdb && cargo build --lib --release
2. Generate bindings: cargo run --bin uniffi-bindgen -- generate --library target/release/libfsdb.dylib --language python --out-dir ../bindings/python
3. Copy library: cp target/release/libfsdb.dylib ../bindings/python/
...
def main():
    print_section("FSDB Python Bindings Example")

    # Create temporary directory for the database
    temp_dir = tempfile.mkdtemp(prefix="fsdb_python_")
    ...
    try:
        ...
        print("  )")
        print("  s3_db = DatabaseOps.create_with_s3('s3://fsdb-test/py_db', schema, s3_config)")
