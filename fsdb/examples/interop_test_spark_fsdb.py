#!/usr/bin/env python3
"""
FSDB ↔ Delta Lake/PySpark Interoperability Test

This script tests full bi-directional compatibility between FSDB and Delta Lake/PySpark:
1. Create a Delta table using PySpark + delta-lake native
2. Read and edit it with FSDB using POSIX commands
3. Verify changes are visible in Spark/Delta

This proves FSDB's "100% Delta Lake compatible" claim.
"""
...
try:
    from fsdb import DatabaseOps, NfsServer, FsdbError
except ImportError as e:
    print(f"✗ Missing FSDB: {e}")
    print("\nInstall with:")
    print("  pip install fsdb-py")
    print("\nOr from source:")
...
def create_spark_session():
    ...
    spark = (SparkSession.builder
        .appName("FSDB-Delta-Interop-Test")
        .master("local[*]")
...
def test_spark_to_fsdb(spark, delta_path):
    """Test 1: Create Delta table with Spark, read with FSDB"""
    print("\n" + "="*70)
    print("TEST 1: Spark → FSDB (Read Spark-created Delta table with FSDB)")
    print("="*70)
    ...
    # Step 2: Read with FSDB
    print("\n2. Reading Delta table with FSDB...")
    try:
        fsdb_db = DatabaseOps.open(delta_path)
        results = fsdb_db.query_json("SELECT * FROM data ORDER BY id")
        
        rows = json.loads(results)
        fsdb_count = len(rows)
        
        print(f"✓ FSDB read {fsdb_count} rows from Spark-created Delta table")
        print(f"  Sample: {rows[0]}")
        
        if fsdb_count == spark_count:
            print("✓ FSDB and Spark row counts match!")
        else:
            print(f"✗ Row count mismatch: FSDB={fsdb_count}, Spark={spark_count}")
            return False
        
        # Step 3: Query with FSDB
        print("\n3. Querying with FSDB SQL...")
        high_earners = fsdb_db.query_json("SELECT name, salary FROM data WHERE salary > 70000 ORDER BY salary DESC")
        earners = json.loads(high_earners)
        print(f"✓ FSDB query found {len(earners)} high earners")
        ...
        print(f"✗ FSDB failed to read Spark Delta table: {e}")
        return False


def test_fsdb_to_spark(spark, delta_path):
    """Test 2: Modify Delta table with FSDB, read changes with Spark"""
    print("\n" + "="*70)
    print("TEST 2: FSDB → Spark (Modify with FSDB, read with Spark)")
    print("="*70)
    
    # Step 1: Insert data with FSDB
    print("\n1. Inserting new data with FSDB...")
    try:
        fsdb_db = DatabaseOps.open(delta_path)
        ...
        print(f"   Inserting via FSDB:")
        for row in new_data:
            print(f"     {row}")
        
        fsdb_db.insert_json(json.dumps(new_data))
        print(f"✓ FSDB inserted {len(new_data)} rows into Delta table")
        
    except FsdbError as e:
        print(f"✗ FSDB insert failed: {e}")
        return False
    
    # Step 2: Verify with Spark
    print("\n2. Verifying FSDB changes with Spark...")
    delta_df = spark.read.format("delta").load(delta_path)
    ...
    try:
        ...
        try:
            ...
            if rm_result.returncode == 0:
                ...
                fsdb_db.insert_json(json.dumps([
...
def test_merge_operations(spark, delta_path):
    ...
    try:
        ...
        print("\n2. Executing MERGE operation via FSDB...")
        print(f"   - UPDATE: Alice (id=1)")
        print(f"   - DELETE: Bob (id=2)")
        print(f"   - INSERT: Ivy (id=9), Jack (id=10)")
        
        # Execute MERGE
        result = fsdb_db.merge_json(json.dumps(merge_data), "id")
...
def test_delta_operations(spark, delta_path):
    ...
    try:
        ...
        print("\n1. Running OPTIMIZE...")
        fsdb_db.optimize()
        print("✓ OPTIMIZE completed via FSDB")
        ...
        fsdb_db.zorder(["age", "salary"])
        print("✓ Z-ORDER completed via FSDB")
        
        # VACUUM (dry run)
        print("\n3. Running VACUUM (dry run)...")
        files = fsdb_db.vacuum_dry_run(0)
...
def main():
    print("="*70)
    print("FSDB ↔ Delta Lake/PySpark Interoperability Test")
    print("="*70)
    print("\nThis test proves FSDB is 100% Delta Lake compatible:")
    print("  • Spark creates Delta table → FSDB reads it")
    print("  • FSDB modifies Delta table → Spark sees changes")
    ...
    temp_dir = tempfile.mkdtemp(prefix="fsdb_interop_")
    ...
    try:
        ...
        results.append(("Spark → FSDB", test_spark_to_fsdb(spark, delta_path)))
        results.append(("FSDB → Spark", test_fsdb_to_spark(spark, delta_path)))
        ...
        print("\n" + "="*70)
        if all_passed:
            print("✓ ALL TESTS PASSED - FSDB IS 100% DELTA LAKE COMPATIBLE!")
        else:
